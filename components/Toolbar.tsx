...
import { Settings, Download, Eye, EyeOff, LayoutTemplate, Printer, Type, Monitor, Edit3, X } from 'lucide-react';
...
interface SettingsToolbarProps {
    ...
    onClose?: () => void;
    // Data props for custom sections
    data?: Resume;
    onUpdateData?: (newData: Resume) => void;
}

export function SettingsToolbar({ settings, onUpdate, onExport, isMobileOpen = false, onClose, data, onUpdateData }: SettingsToolbarProps) {
    ...
    const toggleCustomSection = (index: number) => {
        ...
        if (sections[index]) {
            ...
            onUpdateData({ ...data, customSections: sections });
        }
    };
    ...
    return (
        <>
            ...
            <aside
                id="toolbar"
                className={`w-80 bg-white border-r border-gray-200 h-screen overflow-y-auto fixed left-0 top-0 p-6 flex flex-col gap-8 shadow-[4px_0_24px_rgba(0,0,0,0.02)] z-50 transition-transform duration-300 ease-in-out md:translate-x-0 ${isMobileOpen ? 'translate-x-0' : '-translate-x-full'}`}
            >
                <div className="relative">
                    {onClose && (
                        <button
                            onClick={onClose}
                            className="absolute -top-1 -right-1 p-2 text-gray-400 hover:text-gray-600 md:hidden"
                        >
                            <X className="w-5 h-5" />
                        </button>
                    )}
                        ...
                        <div>
                            ...
                            <div className="space-y-2">
                                ...
                                {(data?.customSections || []).map((section, idx) => (
                                    <button
                                        key={section.id}
                                        onClick={() => toggleCustomSection(idx)}
                                        className="w-full flex items-center justify-between p-2 hover:bg-gray-50 rounded-lg group transition-colors"
                                    >
                                        <span className="text-sm text-gray-600 capitalize truncate text-left pr-2">{section.title}</span>
                                        {section.visible ? (
                                            <Eye className="w-4 h-4 text-indigo-600 shrink-0" />
                                        ) : (
                                            <EyeOff className="w-4 h-4 text-gray-300 group-hover:text-gray-400 shrink-0" />
                                        )}
                                    </button>
                                ))}
                            </div>
